...
InheritableThreadLocal声明的变量是线程私有的，但是子线程可以从父线程继承InheritableThreadLocal声明的变量。
...
线程池内部使用一个变量维护两个值：运行状态(runState)和线程数量 (workerCount)。高3位保存runState，低29位保存workerCount。
...
线程池使用一张Hash表去持有线程的引用，这样可以通过添加引用、移除引用这样的操作来控制线程的生命周期。**那如何去判断一个线程是否是空闲状态：**
...
**线程的最终销毁（回收）是由JVM做的，线程池要做的就是把空闲线程的引用置为空。**

##### 参数动态化

由于线程池的参数设置并没有一个明确清晰的规定，所以在实际的业务场景中很容易出现问题，为了能够快速高效修改线程池参数，采用动态化的方式，把参数放在配置文件中，避免修改后重启服务的问题。
...
![](img/%E7%BA%BF%E7%A8%8B%E6%B1%A0_%E5%8A%A8%E6%80%81%E8%B0%83%E5%8F%82.png)

**负载告警**：

![](img/%E7%BA%BF%E7%A8%8B%E6%B1%A0_%E8%8E%B7%E5%8F%96%E7%BA%BF%E7%A8%8B%E6%B1%A0%E4%BF%A1%E6%81%AF.png)
...
- 三色标记清除算法：用于处理Mark和Sweep之间的Mutation。解决或降低STW。
  ...
  - 漏标：没有把正在引用的对象标记到，这种情况不允许发生。
    ...
    **漏标问题满足以下两个条件：**
    ...
    1.至少有一个黑色对象指向了白色对象：增量更新破坏了这个条件。
    2.所有灰色对象扫描完整个链时，删除之前所有白色对象：初始快照破坏了这个条件。
    
    **写屏障：**漏标的原因是并发过程中引用的改变，C++实现垃圾回收器的时候，通过写屏障将变化的对象引用放入一个队列中，并最终放到一个集合中，然后再**重新标记**阶段进行再次扫描，因为重新标记是STW的，所以可以准确标记，解决漏标问题。
    
    **增量更新：**指的就是我们在并发标记过程中（三色标记之后）一些新的引用的变化情况。
...
底层是使用ReentrantLock实现加锁解锁的，适用于读多写少的并发场景，比如黑白名单、国家城市等基础数据缓存、系统配置等。

缺点：

- 内存占用问题：因为CopyOnWrite的写时复制机制每次进行写操作的时候都会有两个数组对象的内存，如果这个数组对象占用的内存较大的话，如果频繁的进行写入就会造成频繁的Yong GC和Full GC。
- 数据一致性问题：CopyOnWrite容器只能保证数据的最终一致性，不能保证数据的实时一致性。读操作的线程可能不会立即读取到新修改的数据，因为修改操作发生在副本上。但最终修改操作会完成并更新容器所以这是最终一致性。
...
3. **扩容rehash**：扩容的时候 1.7 需要对原数组中的元素进行重新 hash 定位在新数组的位置，1.8 采用更简单的判断逻辑，不需要重新通过哈希函数计算位置，新的位置不变或索引 + 新增容量大小。
   ...
   1.8：只需要看扩容后的数组长度的hashcode最高位是1还是0就好了，是0的话索引没变，是1的话索引变成“原索引+oldCap”。
...
- dubbo支持的协议？
  ...
  Dubbo支持dubbo、rmi、hessian、http、webservice、thrift、redis等多种协议，默认是Dubbo协议。
...
由于索引数量巨大，ES无法直接把全部索引放入内存，转而建立词典索引，构建**有限状态转换器**（FST）放入内存，进一步提高搜索效率。

数据文档的id在词典内的空间消耗也是巨大的，ES使用了**索引帧**（Frame of Reference）技术压缩posting list，带来的压缩效果十分明显。

ES的filter语句采用了**Roaring Bitmap**技术来缓存搜索结果，保证高频filter查询速度的同时降低存储空间消耗。
...
ZooKeeper主要**服务于分布式系统**，可以用ZooKeeper来做：统一配置管理、统一命名服务、分布式锁、集群管理。通过**监听+Znode节点(持久/短暂[临时])**，ZooKeeper完成上述的功能。
...
zookeeper提供了“心跳检测”功能，它会定时向各个服务提供者发送一个请求(实际上建立的是一个 socket **长连接**)，如果长期没有响应，服务中心就认为该服务提供者已经“挂了”，并将其剔除。如果`100.100.0.237`这台机器宕机了，那么zookeeper上的路径就会只剩`/HelloWorldService/1.0.0/100.100.0.238:16888`。
...
在 zk 创建一个 lock 节点，为锁的根节点(持久性节点)

当需要访问共享资源时，先在 lock 节点下创建该共享资源的**临时顺序子节点**；比如共享资源为 A，创建节名字为 A + "lock" + 序号。

在创建成功之后，获取该共享资源(A)的所有节点，如果当前节点是最小节点则表示获得了锁；否则注册上一个节点的删除事件，等待当上一个节点释放锁之后，就获得了资源。

当调用完共享资源后，通过删除删除节点来释放锁。
...
- 垃圾收集器：Serial 收集器（串行、单线程、年轻代）、ParNew 收集器（串行、多线程、年轻代）、Parallel Scavenge 收集器（串行、多线程、吞吐量优先、年轻代）、Serial Old 收集器（串行、单线程、老年代）、Parallel Old 收集器（串行、多线程、注重吞吐量、老年代）、CMS 收集器（并行、多线程、老年代）、G1收集器（并行、多线程、老年代、年轻代）
  ...
  - G1 收集器
    ...
    - G1垃圾回收有young gc和mixed gc，没有full gc。
    - 过程：
      - 初始标记
      - 并发标记
...
两个问题：并发（导致数据库跟缓存结果长期不一致）、数据不一致（数据库跟缓存结果一致有延迟）
...
- Spring事务的传播特性
  ...
  6. PROPAGATION_NESTED：如果一个活动的事务存在，则运行在一个嵌套的事务中. 如果没有活动事务, 则按TransactionDefinition.PROPAGATION_REQUIRED 属性执行。
  7. PROPAGATION_NEVER: 总是非事务地执行，如果存在一个活动事务，则抛出异常；
...
声明式事务（使用@Transactional）的缺点是粒度较大，会作用在整个方法上，编程式事务的作用力度更小，可以只作用于代码行上。
...
- Comparable和Comparator有什么区别？
  ...
  Comparable 接口只有一个方法 compareTo，实现 Comparable 接口并重写 compareTo 方法就可以实现某个类的排序了，它支持 Collections.sort 和 Arrays.sort 的排序。Comparable 是以 -able 结尾的，表示它自身具备着某种能力。
  ...
  实现了Comparable接口的类，需要重写compareTo方法，然后该类自身就具备了排序的功能。

  ------

  Comparator 排序的方法是 compare，Comparator 是以 -or 结尾，表示自身是比较的参与者，而不是本身具备排序的功能。Comparator 除了可以通过创建自定义比较器外，还可以通过匿名类的方式，更快速、便捷的完成自定义比较器的功能。
  ...
  所以用一句话总结二者的区别：Comparable 可以看作是“对内”进行排序接口，而 Comparator 是“对外”进行排序的接口。**Comparator比Comparable的入侵性更小。**
...
- 软件设计原则？
  ...
  - 迪米特原则(最少知道原则)：降低类的耦合，尽可能不关系其他类。
  - 里氏替换原则：子类可以扩展父类的功能，但不能改变父类原有的功能。
  - 合成复用原则：尽量使用对象组合/聚合，而不是继承关系达到软件复用的目的。

- Dubbo常用协议？

  - dubbo://（推荐）利用Netty，TCP传输，单一、异步、长连接，`适合数据量小、高并发`和服务提供者远远少于消费者的场景。
  - rmi://
  - hessian://
  - http://
  - webservice://
